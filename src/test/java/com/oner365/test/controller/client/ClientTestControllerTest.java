...
class ClientTestControllerTest extends BaseControllerTest {
    ...
    void test() {
        ...
        vo.setPassword("1");
        logger.info("request body decode: {}", objectMapper.writeValueAsString(vo));
        String key = Md5Util.getInstance().getMd5(String.valueOf(System.currentTimeMillis()));
        logger.info("key :{}", key);
        String encodeString = Base64.getEncoder().encodeToString(Cipher.encodeSms4(objectMapper.writeValueAsString(vo), key.substring(0, 16).getBytes()));
        Assertions.assertNotNull(encodeString);
        logger.info("request body encode: {}", encodeString);
        
        String sign = RsaUtils.buildRsaEncryptByPublicKey(key, publicKey);
        logger.info("key rsa encode:{}", sign);
        JsonNode result = getWebClient().post()
            .uri(PATH + "/login")
            .header("sign", sign)
            .contentType(MediaType.APPLICATION_JSON)
            .body(BodyInserters.fromValue(encodeString))
            .exchange()
        ...
        String decodeString = Cipher.decodeSms4toString(Base64.getDecoder().decode(result.get("result").asString()), key.substring(0, 16).getBytes());
        logger.info("return result decode: {}", decodeString);
        JsonNode jsonNode = objectMapper.readTree(decodeString.replace("\u0000", ""));
        JsonNode r = jsonNode.get("result");
