package com.oner365.test.controller.client;

import java.util.Base64;

import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.http.MediaType;
import org.springframework.web.reactive.function.BodyInserters;

import com.oner365.data.commons.util.Cipher;
import com.oner365.data.commons.util.Md5Util;
import com.oner365.data.commons.util.RsaUtils;
import com.oner365.sys.vo.LoginUserVo;
import com.oner365.test.controller.BaseControllerTest;

import tools.jackson.databind.JsonNode;

/**
 * Test ClientTestController
 *
 * @author liutao
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class ClientTestControllerTest extends BaseControllerTest {

    private static final String PATH = "/client";

    public static String publicKey = "";

    public static String privateKey = "";

    @Test
    void test() {
        LoginUserVo vo = new LoginUserVo();
        vo.setUserName("admin");
        vo.setPassword("1");
        logger.info("request body decode: {}", objectMapper.writeValueAsString(vo));
        String key = Md5Util.getInstance().getMd5(String.valueOf(System.currentTimeMillis()));
        logger.info("key :{}", key);
        String encodeString = Base64.getEncoder()
            .encodeToString(Cipher.encodeSms4(objectMapper.writeValueAsString(vo), key.substring(0, 16).getBytes()));
        Assertions.assertNotNull(encodeString);
        logger.info("request body encode: {}", encodeString);

        String sign = RsaUtils.buildRsaEncryptByPublicKey(key, publicKey);
        logger.info("key rsa encode:{}", sign);
        JsonNode result = getWebClient().post()
            .uri(PATH + "/login")
            .header("sign", sign)
            .contentType(MediaType.APPLICATION_JSON)
            .body(BodyInserters.fromValue(encodeString))
            .exchange()
            .expectBody(JsonNode.class)
            .returnResult()
            .getResponseBody();
        Assertions.assertNotNull(result);
        logger.info("return result: {}", result);
        String decodeString = Cipher.decodeSms4toString(Base64.getDecoder().decode(result.get("result").asString()),
                key.substring(0, 16).getBytes());
        logger.info("return result decode: {}", decodeString);
        JsonNode jsonNode = objectMapper.readTree(decodeString.replace("\u0000", ""));
        JsonNode r = jsonNode.get("result");
        logger.info("return result decode: {}", r);
    }

}
