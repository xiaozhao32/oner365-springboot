...
public class GenUtils {
   ...
   * 数据库数字类型
   */
  protected static final String[] COLUMN_TYPE_NUMBER = { "tinyint", "smallint", "mediumint", "int", "number", "integer",
  ...
  public static String replaceFirst(String replacement, String[] searchList) {
    return Arrays.stream(searchList).filter(replacement::startsWith).findFirst()
        .map(searchString -> replacement.replaceFirst(searchString, "")).orElse(replacement);
  }
