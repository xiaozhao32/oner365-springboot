...
public class JobInvokeUtil {
  ...
  public static List<Object[]> getMethodParams(String invokeTarget) {
    ...
    Arrays.stream(methodParams).map(StringUtils::trimToEmpty).forEach(str -> {
      ...
      else if (StringUtils.equals(str, Boolean.TRUE.toString()) || StringUtils.equalsIgnoreCase(str, Boolean.FALSE.toString())) {
        classList.add(new Object[]{Boolean.valueOf(str), Boolean.class});
      }
      // long长整形，包含L
      else if (StringUtils.containsIgnoreCase(str, SYMBOL_LONG)) {
        classList.add(new Object[]{Long.valueOf(StringUtils.replaceIgnoreCase(str, SYMBOL_LONG, "")), Long.class});
      }
      // double浮点类型，包含D
      else if (StringUtils.containsIgnoreCase(str, SYMBOL_DOUBLE)) {
        classList.add(new Object[]{Double.valueOf(StringUtils.replaceIgnoreCase(str, SYMBOL_DOUBLE, "")), Double.class});
      }
      // 其他类型归类为整形
      else {
